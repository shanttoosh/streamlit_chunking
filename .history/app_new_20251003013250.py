...
def display_preview_tables(preview_data):
    ...
    if "dtype_preview" in preview_data:
    ...
    if "null_preview" in preview_data:
        ...
        with st.container():
            ...
            with col3:
                if config_type == "Null Handling" and configs[col] == "fill":
                    configs[f"{col}_fill_method"] = st.selectbox(
                        f"Fill method",
                        ["median", "mode", "mean", "custom"],
                        key=f"{key_prefix}_fill_method_{col}",
                        help=f"Choose how to calculate fill value"
                    )
                    if configs[f"{col}_fill_method"] == "custom":
                        configs[f"{col}_custom_value"] = st.text_input(
                            f"Custom value",
                            value="Unknown",
                            key=f"{<｜tool▁sep｜>key_prefix}_custom_{col}",
...
def preview_csv_data(file_content: bytes, filename: str):
    files = {"file": (filename, file_content, "text/csv")}
    response = requests.post(f"{API_BASE_URL}/preview/data", files=files)
    return response.json()
...
def preview_db_data(db_type: str, host: str, port: int, username: str, password: str, database: str, table_name: str):
    ...
    response = requests.post(f"{API_BASE_URL}/preview/data", data=data)
    return response.json()
...
if st.session_state.selected_mode and st.session_state.data_source == "csv" and hasattr(st.session_state, 'uploaded_file') and st.session_state.uploaded_file is not None:
    ...
    if st.session_state.selected_mode == "fast":
        ...
        preview_data = None
        if st.session_state.data_source == "csv" and hasattr(st.session_state, 'csv_preview'):
            preview_data = st.session_state.csv_preview
        elif st.session_state.data_source == "database" and hasattr(st.session_state, 'db_preview'):
        ...
        with st.expander("⚙️ Config-1 Settings", expanded=True):
            ...
            with col1:
                ...
                if preview_data and "null_preview" in preview_data:
                    st.write("Configure null handling for each column:")
                    columns = [item["column_name"] for item in preview_data["null_preview"]["data"]]
        ...
        with st.expander("🔬 Deep Mode Settings", expanded=True):
            ...
            with tab1:
                ...
                if preview_data and "dtype_preview" in preview_data:
                    st.write("Convert data types for each column:")
                    columns = [item["column_name"] for item in preview_data["dtype_preview"]["data"]]
