...
# -----------------------------
def document_based_chunking_multi(
    df: pd.DataFrame,
    ...
    """
    Group rows by a combination of key columns and chunk by token_limit.
    Returns a list of chunk texts and a parallel list of metadata dicts.
    """
    start_time = time.time()
    ...
    for key_values, group in grouped:
        ...
        if token_count <= token_limit:
            ...
            avg_tokens_per_row = max(1.0, token_count / max(1, len(group)))
            rows_per_chunk = max(1, min(len(group), int(token_limit / avg_tokens_per_row)))
            if rows_per_chunk <= 0:
                rows_per_chunk = 1
    ...
    logger.info(f"Document-based multi-key chunking completed in {time.time() - start_time:.2f}s, created {len(chunks)} chunks")
    return chunks, metas
...
def run_chunking(
    ...
    *,
    ...
    """
    Unified entry-point to run any chunking method and return (chunks, metadata).
    method ∈ {"fixed", "recursive_kv", "semantic", "document", "document_multi"}
    """
    method = (method or "").strip().lower()

    if method == "fixed":
        texts = chunk_fixed(df, chunk_size=chunk_size, overlap=overlap)
        metas = [{
            'chunk_index': i,
            'chunking_method': 'fixed',
            'chunk_size_param': chunk_size,
            'overlap_param': overlap,
            'token_count': estimate_token_count(t)
        } for i, t in enumerate(texts)]
        return texts, metas

    if method == "recursive_kv":
        texts = chunk_recursive_keyvalue(df, chunk_size=chunk_size, overlap=overlap)
        metas = [{
            'chunk_index': i,
            'chunking_method': 'recursive_kv',
            'chunk_size_param': chunk_size,
            'overlap_param': overlap,
            'token_count': estimate_token_count(t)
        } for i, t in enumerate(texts)]
    ...
    if method == "semantic":
        texts = chunk_semantic_cluster(df, n_clusters=n_clusters)
        metas = [{
            'chunk_index': i,
            'chunking_method': 'semantic_cluster',
    ...
    if method == "document":
        if not key_column:
            raise ValueError("'key_column' is required for document chunking")
        texts, metas = document_based_chunking(
            df, key_column=key_column, token_limit=token_limit, preserve_headers=preserve_headers
        )
        return texts, metas

    if method == "document_multi":
        if not key_columns or len(key_columns) == 0:
            raise ValueError("'key_columns' must be a non-empty list for multi-key document chunking")
        texts, metas = document_based_chunking_multi(
            df, key_columns=key_columns, token_limit=token_limit, preserve_headers=preserve_headers
        )
        return texts, metas
