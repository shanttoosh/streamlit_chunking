...
def store_faiss_enhanced(chunks: List[str], embeddings: np.ndarray, metadata: List[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Enhanced FAISS storage with metadata support"""
    start_time = time.time()
    try:
        import faiss
        import pickle
    ...
    faiss_data = {
        "documents": chunks,
        "metadata": metadata or [{"chunk_id": str(i)} for i in range(len(chunks))],
    ...
    logger.info(f"Enhanced FAISS storage completed in {time.time() - start_time:.2f}s, stored {embeddings.shape[0]} vectors")
