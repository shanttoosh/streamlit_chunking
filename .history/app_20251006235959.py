...
if st.session_state.current_mode:
    if st.session_state.current_mode == "fast":
        ...
        with col1:
            # Null handling removed for Config-1
        ...
        with col2:
            if 'chunk_method' in locals() and chunk_method == "document":
                ...
                st.info("Headers will NOT be included in each chunk.")
            ...
            # Retrieval metric selection (Config-1)
            ...
            if storage_choice == "chromadb":
                st.caption("Note: ChromaDB typically uses cosine distance; changing metric requires creating collection with a different space.")
            else:
                st.caption("Note: Current FAISS index is L2 (euclidean). Other metrics require building a different index.")
        ...
        if uploaded_file:
            ...
            if st.session_state.deep_config_step == 9:
                ...
                if storage_choice == "ChromaDB":
                    st.caption("Note: ChromaDB typically uses cosine distance; selection is informational unless collection space differs.")
                else:
                    st.caption("Note: Current FAISS index is L2 (euclidean). Other metrics would require a different index build.")
