...
def call_deep_config_api(file_path: str, filename: str, config_dict: dict, db_config: dict = None,
                        use_openai: bool = False, openai_api_key: str = None, openai_base_url: str = None,
                        use_turbo: bool = False, batch_size: int = 64):
    """Send CSV upload or trigger DB import for Deep Config mode"""
    try:
        ...
        if db_config and db_config.get('use_db'):
            data = {
                ...
                "database": db_config.get("database"),
                "table_name": db_config.get("table_name"),
                "preprocessing_config": json.dumps(config_dict.get("preprocessing", {})),
                "chunking_config": json.dumps(config_dict.get("chunking", {})),
                "embedding_config": json.dumps(config_dict.get("embedding", {})),
                "storage_config": json.dumps(config_dict.get("storage", {})),
                "use_openai": use_openai,
                "openai_api_key": openai_api_key,
                "openai_base_url": openai_base_url,
                "use_turbo": use_turbo,
            ...
            response = requests.post(f"{API_BASE_URL}/run_deep_config", data=data)
            return response.json()
        ...
        with open(file_path, 'rb') as f:
            ...
            response = requests.post(f"{API_BASE_URL}/run_deep_config", files=files, data=data)
            return response.json()
    except Exception as e:
        return {"error": f"Deep Config API call failed: {str(e)}"}

def call_deep_config_db_import_api(db_config: dict):
    """Import data from database for Deep Config mode (legacy function - now integrated into call_deep_config_api)"""
    try:
        data = {
            "db_type": db_config.get("db_type"),
            "host": db_config.get("host"),
        ...
        response = requests.post(f"{API_BASE_URL}/db/import_one", data=data)
        return response.json()
    except Exception as e:
        return {"error": f"DB import API call failed: {str(e)}"}
    ...
    if st.session_state.current_mode == "fast":
        ...
        if uploaded_file or (input_source == "🗄️ Database Import" and use_db_config):
            if st.session_state.deep_config_step == 0:
                if uploaded_file:
                    ...
                    with st.spinner("🔄 Loading data from database..."):
                        ...
                        import_result = call_deep_config_db_import_api(use_db_config)
                        
                        if "error" in import_result:
                            st.error(f"❌ Database import failed: {import_result['error']}")
                            st.session_state.deep_df = pd.DataFrame()
                        else:
                            # For Deep Config, we need to get the raw data, not processed results
                            # We'll use a different approach - load the data directly
                            try:
            ...
            if st.session_state.deep_config_step == 9:
                ...
                # Add API-based pipeline execution option
                st.markdown("---")
                st.subheader("🚀 Complete Pipeline Execution (API-based)")
                
                # Check if we have data loaded
                has_data = (uploaded_file is not None) or (input_source == "🗄️ Database Import" and use_db_config is not None)
                
                if has_data:
                    col1, col2 = st.columns([2, 1])
                    with col1:
                        st.info("💡 **Quick Option**: Run the complete Deep Config pipeline using the API (recommended for consistency with other modes)")
                    with col2:
                        if st.button("🚀 Run Complete Deep Config Pipeline", type="primary", use_container_width=True):
                            with st.spinner("Running complete Deep Config pipeline via API..."):
                                try:
                                    # Prepare configuration
                                    config_dict = {
                                        "preprocessing": st.session_state.get("preprocessing_config", {}),
                                        "chunking": st.session_state.get("chunking_config", {}),
                                        "embedding": st.session_state.get("embedding_config", {}),
                                        "storage": st.session_state.get("storage_config", {})
                                    }
                                    ...
                                    if uploaded_file:
                                        ...
                                        try:
                                            ...
                                            result = call_deep_config_api(
                                                temp_path,
                                                uploaded_file.name,
                                                config_dict,
                                                None,  # No DB config for CSV
                                                st.session_state.get("use_openai", False),
                                                st.session_state.get("openai_api_key"),
                                                st.session_state.get("openai_base_url"),
                                                st.session_state.get("use_turbo", False),
                                        ...
                                        # DB import
                                        result = call_deep_config_api(
                                            None, None, config_dict, use_db_config,
                                            st.session_state.get("use_openai", False),
                                            st.session_state.get("openai_api_key"),
                                            st.session_state.get("openai_base_url"),
                                            st.session_state.get("use_turbo", False),
                                    ...
                                    if "error" in result:
                                        ...
                                        st.success("✅ Complete Deep Config pipeline executed successfully!")
                                        if 'summary' in result:
                                            summary = result['summary']
                                            st.info(f"📊 **Results**: {summary.get('rows', 'N/A')} rows → {summary.get('chunks', 'N/A')} chunks → {summary.get('stored', 'N/A')} storage")
                    ...
                    st.warning("⚠️ Please load data (CSV file or database) before running the pipeline")
                
                st.markdown("---")
                st.subheader("Vector Storage & Retrieval (Step-by-step)")
