...
<style>
    ...
    h1, h2, h3, h4, h5, h6 {
        ...
        border-left: 3px solid var(--ev-colors-highlight) !important;
    ...
    .stButton > button:hover {
        ...
        border-color: var(--ev-colors-highlight) !important;
    }
    
    /* Primary buttons use highlight color */
    ...
    .process-step {
        ...
        border-radius: 4px;
        margin: 6px 0;
        border-left: 3px solid var(--ev-colors-secondary);
        transition: all 0.2s ease;
    }
    ...
    .stTextArea > div > div > textarea:focus {
        border-color: var(--ev-colors-highlight) !important;
        box-shadow: 0 0 0 2px rgba(231, 95, 51, 0.2) !important;
    }
    
    /* Uniform checkboxes & radio buttons */
    .stCheckbox > label, 
    .stRadio > label {
        color: var(--ev-colors-text) !important;
        font-size: 0.9em !important;
    ...
    .css-17eq0hr {
        background: var(--ev-colors-primary) !important;
        border-right: 1px solid var(--ev-colors-borderColor) !important;
    }
    ...
    .stSuccess {
        ...
        border-left: 3px solid var(--ev-colors-success) !important;
        border-radius: 4px !important;
    ...
    .stError {
        ...
        border-left: 3px solid var(--ev-colors-danger) !important;
        border-radius: 4px !important;
        padding: 8px 12px !important;
    }
    ...
    .stInfo {
        background: var(--ev-colors-primary) !important;
        color: var(--ev-colors-text) !important;
        border-left: 3px solid var(--ev-colors-secondary) !important;
        border-radius: 4px !important;
        padding: 8px 12px !important;
    }
    ...
    .stProgress > div > div > div {
        background-color: var(--ev-colors-highlight) !important;
    }
    
    /* Compact scrollable chunk display */
    .scrollable-chunk {
        background: var(--ev-colors-primary);
        border: 1px solid var(--ev-colors-borderColor);
        border-radius: 4px;
    ...
    .stTabs [data-baseweb="tab-list"] {
        background: var(--ev-colors-primary) !important;
        border-bottom: 1px solid var(--ev-colors-borderColor) !important;
    }
    ...
    .stTabs [aria-selected="true"] {
        background: var(--ev-colors-primary) !important;
        color: var(--ev-colors-highlight) !important;
        border-bottom: 2px solid var(--ev-colors-highlight) !important;
    }
    
    /* Uniform download buttons */
    .download-section {
        background: var(--ev-colors-primary);
        border: 1px solid var(--ev-colors-borderColor);
        border-radius: 6px;
    ...
    hr {
        border: none !important;
        border-top: 1px solid var(--ev-colors-borderColor) !important;
...
def call_deep_config_duplicates_api(strategy: str):
    """Step 3: Handle duplicate rows"""
    try:
        data = {"strategy": strategy}
        response = requests.post(f"{API_BASE_URL}/deep_config/duplicates", data=data)
        return response.json()
    except Exception as e:
...
def call_deep_config_chunk_api(chunk_params: dict):
    ...
    try:
        ...
        store_metadata = chunk_params.get("store_metadata", False)
        numeric_columns = chunk_params.get("numeric_columns", 0)
        categorical_columns = chunk_params.get("categorical_columns", 0)
...
if st.session_state.current_mode:
    if st.session_state.current_mode == "fast":
        ...
        if st.button("🚀 Run Fast Pipeline", type="primary", type="primary", use_container_width=True, disabled=not run_enabled):
        ...
        if uploaded_file or (input_source == "🗄️ Database Import" and use_db_config):
            ...
            if st.session_state.deep_config_step == 1:
                ...
                def get_smart_suggestion(col_name, col_data):
                    ...
                    if col_data.dtype == 'object':
                        sample_values = col_data.dropna().head(20)  # Increased sample size
                        ...
                        for val in sample_values:
                            ...
                            if (any(char in val_str for char in ['-', '/', ':']) and 
                                len(val_str) > 8 and 
                                (val_str.count('-') >= 2 or val_str.count('/') >= 2 or ':' in val_str)):
                                date_patterns.append(val_str)
                        
                        if len(date_patterns) > len(sample_values) * 0.6:
                            return 'datetime'
                        ...
                        # Check for numeric patterns (but not IDs)
                        numeric_values = []
                        for val in sample_values:
                            try:
                        ...
                        # If mostly numeric and not an ID column, suggest float64
                        if (len(numeric_values) > len(sample_values) * 0.8 and 
                            not any(word in col_name_lower for word in ['id', 'key', 'code', 'ref'])):
                            return 'float64'
                        
                        # Check for categorical data (limited unique values)
                        unique_ratio = col_data.nunique() / len(col_data)
                        if unique_ratio < 0.1 and col_data.nunique() < 50:  # Less than 10% unique values and less than 50 categories
                            return 'object'  # Keep as categorical
                        
                        return 'object'
                    
                    # Handle numeric columns
                    elif str(col_data.dtype) == 'int64':
                        # Check if it's actually an ID (high cardinality) or a count/measure
                        unique_ratio = col_data.nunique() / len(col_data)
                        if (unique_ratio > 0.8 and 
                            any(word in col_name_lower for word in ['id', 'key', 'code', 'ref', 'number'])):
                            return 'object'  # High cardinality IDs should be object
                        else:
                            return 'int64'  # Counts, scores, etc. stay as int64
            ...
            if st.session_state.deep_config_step == 2:
                ...
                # Simplified individual controls with bulk actions
                st.write("📋 **Null Handling Strategies:**")
                
                null_strategies = {}
                options = ["No change", "drop", "mean", "median", "mode", "zero", "unknown", "ffill", "bfill"]
                
                # Bulk action buttons
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    if st.button("Apply Suggestions to All", key="apply_all_suggestions"):
                        for _, row in filtered_profile.iterrows():
                            col_name = row['column']
                            suggestion = row['suggested']
                            if suggestion in options and suggestion != "No change":
                                null_strategies[col_name] = suggestion
                        st.success("✅ Applied suggestions to all columns!")
                
                with col2:
                    if st.button("Apply Median to Numeric", key="apply_median_numeric"):
                        for _, row in filtered_profile.iterrows():
                            if row['dtype'] in ['int64', 'float64'] and row['column'] not in null_strategies:
                                null_strategies[row['column']] = 'median'
                        st.success("✅ Applied median to numeric columns!")
                
                with col3:
                    if st.button("Apply Mode to Text", key="apply_mode_text"):
                        for _, row in filtered_profile.iterrows():
                            if row['dtype'] == 'object' and row['column'] not in null_strategies:
                                null_strategies[row['column']] = 'mode'
                        st.success("✅ Applied mode to text columns!")
                
                with col4:
                    if st.button("Clear All", key="clear_all_strategies"):
                        null_strategies.clear()
                        st.success("✅ Cleared all strategies!")
                
                st.divider()
                
                # Individual column controls
                st.write("**Individual Column Controls:**")
                cols_per_row = 2
                for i in range(0, len(filtered_profile), cols_per_row):
                    cols = st.columns(cols_per_row)
                    for j, (_, row) in enumerate(filtered_profile.iloc[i:i+cols_per_row].iterrows()):
                        with cols[j]:
                            col_name = row['column']
                            suggestion = row['suggested']
                            ...
                            st.write(f"**{col_name}** ({row['dtype']})")
                            st.write(f"Nulls: {null_count} ({null_pct:.1f}%) | Suggested: {suggestion}")
                            
                            # Get current selection or use suggestion
                            current_selection = null_strategies.get(col_name, suggestion if suggestion in options else "No change")
                            ...
                            choice = st.selectbox(
                                ...
                                options,
                                index=current_index,
                                key=f"deep_null_choice_{col_name}"
                            )
                            
                            if choice != "No change":
                                null_strategies[col_name] = choice
                            elif col_name in null_strategies:
                                del null_strategies[col_name]
                    ...
                    with st.spinner("🔄 Applying null handling via API..."):
                        try:
                            ...
                            if "error" in result:
                                ...
                                st.session_state.deep_file_meta = result.get('file_info', {})
                                st.session_state.deep_numeric_meta = []
                                st.session_state.deep_config_step = 3
                                st.rerun()
            ...
            # Step 3: Duplicate Row Analysis/Removal
            if st.session_state.deep_config_step == 3:
                st.subheader("Duplicate Row Analysis & Removal")
                # Back button
                ...
                # Check if data is available
                if st.session_state.deep_df.empty:
                    st.error("❌ No data available for duplicate analysis. Please go back and load data first.")
                    if st.button("Back to Data Loading", key="deep_back_to_data_loading_step3"):
                        st.session_state.deep_config_step = 0
                        st.rerun()
                    st.stop()
                ...
                duplicate_analysis = analyze_duplicates_enhanced(st.session_state.deep_df)
                
                # Display duplicate analysis
                st.write("📊 **Duplicate Analysis Results:**")
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric("Total Rows", duplicate_analysis['total_rows'])
                with col2:
                    st.metric("Duplicate Rows", duplicate_analysis['duplicate_rows_count'])
                with col3:
                    st.metric("Duplicate Groups", duplicate_analysis['unique_duplicate_groups'])
                with col4:
                    st.metric("Duplicate %", f"{duplicate_analysis['duplicate_percentage']:.1f}%")
                
                if duplicate_analysis['has_duplicates']:
                    st.warning(f"⚠️ Found {duplicate_analysis['duplicate_rows_count']} duplicate rows ({duplicate_analysis['duplicate_percentage']:.1f}% of data)")
                    
                    # Show duplicate groups (first 5)
                    if duplicate_analysis['duplicate_groups']:
                        st.write("🔍 **Sample Duplicate Groups:**")
                        for i, group in enumerate(duplicate_analysis['duplicate_groups'][:5]):
                            with st.expander(f"Group {i+1}: {group['count']} identical rows"):
                                st.write("**Values:**")
                                for col, val in group['values'].items():
                                    st.write(f"- {col}: {val}")
                                st.write(f"**Row indices:** {group['indices']}")
                    
                    # Duplicate handling options
                    st.write("🛠️ **Duplicate Handling Options:**")
                    strategy = st.radio(
                        ...
                        [
                            ...
                            "Keep All Duplicates (No Action)"
                        ],
                        index=0,
                        key="deep_duplicate_strategy"
                    )
                    
                    # Map strategy to backend values
                    strategy_map = {
                        "Keep First Occurrence (Recommended)": "keep_first",
                        "Keep Last Occurrence": "keep_last",
                        "Remove All Duplicates": "remove_all",
                        "Keep All Duplicates (No Action)": "keep_all"
                    }
                    ...
                    if selected_strategy != "keep_all":
                        if selected_strategy == "keep_first":
                            rows_after = duplicate_analysis['total_rows'] - duplicate_analysis['unique_duplicate_groups']
                        elif selected_strategy == "keep_last":
                            rows_after = duplicate_analysis['total_rows'] - duplicate_analysis['unique_duplicate_groups']
                        elif selected_strategy == "remove_all":
                            rows_after = duplicate_analysis['total_rows'] - duplicate_analysis['duplicate_rows_count']
                        
                        st.info(f"📈 **Preview**: {duplicate_analysis['total_rows']} rows → {rows_after} rows (removing {duplicate_analysis['total_rows'] - rows_after} duplicates)")
                    
                    # Apply button
                    if st.button("Apply Duplicate Removal", key="deep_apply_duplicates"):
                        with st.spinner("🔄 Processing duplicates via API..."):
                            try:
                                result = call_deep_config_duplicates_api(selected_strategy)
                                
                                if "error" in result:
                                    st.error(f"❌ Duplicate removal failed: {result['error']}")
                                else:
                                    st.success("✅ Duplicate removal completed successfully!")
                                    st.info(f"📊 **Result**: {result['rows_before']} rows → {result['rows_after']} rows (removed {result['duplicates_removed']} duplicates)")
                    ...
                    # Skip button
                    if st.button("Skip Duplicate Removal", key="deep_skip_duplicates"):
                        st.info("Duplicate removal skipped.")
                        st.session_state.deep_config_step = 4
                        st.rerun()
                        
                else:
                    st.success("✅ No duplicate rows found in the dataset!")
                    if st.button("Proceed to Stop Words Removal", key="deep_proceed_no_duplicates"):
                        st.session_state.deep_config_step = 4
                        st.rerun()
            ...
            if st.session_state.deep_config_step == 4:
                ...
                # Back button
                if st.button("Back to Duplicate Analysis", key="deep_back_to_duplicates"):
                    st.session_state.deep_chunking_result = None
                    st.session_state.deep_embedding_result = None
                    st.session_state.deep_config_step = 3
                    st.rerun()
            ...
            if st.session_state.deep_config_step == 6:
                ...
                storage_choice = st.radio(
                    ...
                    help="Select the vector database for storing embeddings and metadata"
                )
                ...
                if store_metadata:
                    ...
                    if st.button("Apply Metadata Selection", key="deep_apply_metadata"):
                        ...
                        if storage_choice == "ChromaDB":
                            ...
                            st.success("Metadata selection saved. These fields will be stored with chunks in FAISS.")
                else:
                    if storage_choice == "ChromaDB":
                        ...
                        try:
                            ...
                            if chunking_method == "Fixed Size":
                                chunk_params = {
                                    ...
                                    "numeric_columns": len(st.session_state.deep_meta_numeric_cols),
                                    "categorical_columns": len(st.session_state.deep_meta_categorical_cols)
                                }
                            elif chunking_method == "Recursive":
                                chunk_params = {
                                    "method": "recursive",
