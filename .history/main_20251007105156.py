...
async def deep_config_chunk(
    ...
    try:
        ...
        if chunk_method == "fixed":
            ...
            # Get n_clusters from form data
            n_clusters = 10  # default value
            try:
                from fastapi import Request
                # This is a workaround since we can't access request directly in this context
                # We'll need to pass it as a parameter
                n_clusters = int(request.form.get("n_clusters", 10)) if 'request' in locals() else 10
            except:
                n_clusters = 10
