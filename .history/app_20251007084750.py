            ...
            if st.session_state.deep_config_step == 0:
                if uploaded_file:
                    ...
                    with st.spinner("🔄 Loading data from database..."):
                        # Create a temporary API call to load DB data
                        # We'll implement this in the backend
                        st.session_state.deep_db_config = use_db_config
                        st.session_state.deep_df = pd.DataFrame()  # Placeholder
                        st.session_state.deep_file_info = {
                            "source": f"db:{use_db_config['db_type']}",
                            "table": use_db_config['table_name'],
                            "database": use_db_config['database']
                        }
                        st.info("Database import will be processed during pipeline execution")
