...
async def deep_config_preprocess(
    ...
    try:
        ...
        from backend import validate_and_normalize_headers, normalize_text_column
        
        # Step 1: Normalize headers
        df = validate_and_normalize_headers(df)
        
        # Step 2: Normalize text columns (HTML removal, lowercase, strip)
...
async def deep_config_duplicates(
    ...
    try:
        ...
        duplicate_analysis = analyze_duplicates_enhanced(current_df)
        
        # Apply duplicate removal strategy
        df_processed = remove_duplicates_enhanced(current_df, strategy)
        ...
        return {
            "status": "success",
            "rows_before": duplicate_analysis['total_rows'],
            "rows_after": len(df_processed),
            "duplicates_removed": duplicate_analysis['total_rows'] - len(df_processed),
        ...
        if store_metadata and metadata:
            ...
            for i, meta in enumerate(metadata):
                ...
                if numeric_columns > 0:
                    numeric_cols = current_df.select_dtypes(include=['number']).columns.tolist()[:numeric_columns]
                    for col in numeric_cols:
                        if col in current_df.columns:
                            # For document-based chunking, filter by key_value
                            if chunk_method == "document" and key_column and meta.get('key_value'):
                                col_data = current_df[current_df[key_column] == meta.get('key_value')][col]
                            else:
                ...
                if categorical_columns > 0:
                    categorical_cols = current_df.select_dtypes(include=['object']).columns.tolist()[:categorical_columns]
...
async def retrieve_with_metadata(
    ...
    try:
        ...
        if current_store_info["type"] == "faiss":
            ...
            return {"error": f"Unsupported store type: {current_store_info['type']}"}
        ...
        return {
            ...
            "store_type": current_store_info["type"]
        }
    ...
    try:
        ...
        if operation == "fast":
            return await internal_run_fast(file, db_type, host, port, username, password,
                                         database, table_name, use_openai, openai_api_key,
            ...
            return await internal_run_config1(file, db_type, host, port, username, password,
                                            ...
                                            use_openai, openai_api_key, openai_base_url,
            ...
            return await internal_db_import(db_type, host, port, username, password, database,
                                          ...
                                          storage_choice, use_openai, openai_api_key, openai_base_url,
...
async def internal_run_fast(file, db_type, host, port, username, password, database, table_name,
                           use_openai, openai_api_key, openai_base_url, process_large_files, use_turbo, batch_size):
...
async def internal_deep_config_step(step, file, db_type, host, port, username, password, database, table_name,
    ...
    if step == "preprocess":
        ...
        return await deep_config_embed(model_choice, use_openai=False, openai_api_key=None,
                                      openai_base_url=None, batch_size=batch_size, use_parallel=use_turbo)
    elif step == "store":
...
async def internal_export(export_type):
    ...
    try:
        if export_type == "chunks":
            return await export_chunks()
        elif export_type == "embeddings":
            return await export_embeddings()
        elif export_type == "embeddings_text":
            return await export_embeddings_text()
        elif export_type == "preprocessed":
            return await deep_config_export_preprocessed()
        elif export_type == "deep_chunks":
            return await deep_config_export_chunks()
        elif export_type == "deep_embeddings":
            return await deep_config_export_embeddings()
        else:
...
async def internal_db_import(db_type, host, port, username, password, database, table_name,
                            ...
                            use_openai, openai_api_key, openai_base_url, use_turbo, batch_size):
    ...
    try:
        return await db_import_one(db_type, host, port, username, password, database, table_name,
                                 "config1", use_openai, openai_api_key, openai_base_url, use_turbo, batch_size)
    except Exception as e:
        return {"error": str(e)}
