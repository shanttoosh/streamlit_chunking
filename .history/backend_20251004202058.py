...
BATCH_SIZE = 1000  # Rows per batch for large files
...
def import_large_table_to_dataframe(conn, table_name: str, chunk_size: int = 10000):
    ...
    while True:
        ...
        if len(chunks) * chunk_size > 50000:  # Process every 50k rows
...
# -----------------------------
# 🔹 Enhanced Preprocessing Pipeline with Large File Support
# -----------------------------
...
def preprocess_default(df: pd.DataFrame):
    """
    Default preprocessing: header validation and basic cleaning
    """
    start_time = time.time()
...
def analyze_null_values(df: pd.DataFrame):
    """
...
def handle_null_values_per_column(df: pd.DataFrame, column: str, strategy: str, custom_value=None):
    """
    Handle null values for a specific column based on strategy
    """
    df_copy = df.copy()
    
    if strategy == "Skip / Leave as is":
        return df_copy, f"Skipped null handling for '{column}'. No changes made."
    
    try:
        if strategy == "Drop Rows":
            df_copy = df_copy.dropna(subset=[column])
            message = f"Dropped rows with null values in '{column}'"
            
        elif strategy == "Fill with Mean" and pd.api.types.is_numeric_dtype(df_copy[column]):
            df_copy[column] = df_copy[column].fillna(df_copy[column].mean())
            message = f"Filled null values in '{column}' with mean"
            
        elif strategy == "Fill with Median" and pd.api.types.is_numeric_dtype(df_copy[column]):
            df_copy[column] = df_copy[column].fillna(df_copy[column].median())
            message = f"Filled null values in '{column}' with median"
            
        elif strategy == "Fill with Mode":
            mode_val = df_copy[column].mode()
            if not mode_val.empty:
                df_copy[column] = df_copy[column].fillna(mode_val.iloc[0])
                message = f"Filled null values in '{column}' with mode"
            else:
                message = f"No mode found for '{column}', no changes made"
                
        elif strategy == "Fill with Custom Value" and custom_value is not None:
            if pd.api.types.is_datetime64_any_dtype(df_copy[column]):
                parsed_val = pd.to_datetime(custom_value)
                df_copy[column] = df_copy[column].fillna(parsed_val)
                message = f"Filled null values in '{column}' with custom datetime: {custom_value}"
            elif pd.api.types.is_numeric_dtype(df_copy[column]):
                parsed_val = float(custom_value)
                df_copy[column] = df_copy[column].fillna(parsed_val)
                message = f"Filled null values in '{column}' with custom numeric: {custom_value}"
            else:
                df_copy[column] = df_copy[column].fillna(custom_value)
                message = f"Filled null values in '{column}' with custom value: {custom_value}"
        else:
            message = f"Invalid strategy or missing custom value for '{column}'"
            
    except Exception as e:
        message = f"Error during null handling for '{column}': {str(e)}"
        return df, message
    
    return df_copy, message

def handle_duplicates(df: pd.DataFrame, strategy: str = "remove"):
    """
    ...
    if dup_count == 0:
        return df, f"No duplicate rows found"
    
    if strategy == "remove":
        df_cleaned = df.drop_duplicates(keep="first")
        message = f"Removed {dup_count} duplicate rows"
        return df_cleaned, message
    else:
        message = f"Found {dup_count} duplicate rows, keeping all"
        return df, message
...
def remove_stopwords_from_text_columns(df: pd.DataFrame, remove_stopwords: bool = True):
    ...
    try:
        ...
        return df, "NLTK not available for stopwords removal"
    ...
    logger.info(f"Stopwords removal completed for {len(processed_cols)} columns")
    return df_copy, message
...
def apply_text_normalization(df: pd.DataFrame, method: str = "lemmatize"):
    ...
    try:
        ...
        nltk_available = False
        logger.warning("NLTK not available for text normalization")
        return df, "NLTK not available for text normalization"
    ...
    logger.info(f"Text normalization ({method}) completed for {len(text_cols)} columns")
    return df_copy, message

def preprocess_enhanced_pipeline(df: pd.DataFrame, 
                                type_conversions: Dict[str, str] = None,
                                null_handling_strategies: Dict[str, Dict] = None,
                                handle_duplicates_flag: bool = True,
                                remove_stopwords_flag: bool = False,
                                normalization_method: str = "none"):
    """
    Enhanced preprocessing pipeline combining all preprocessing steps
    """
    start_time = time.time()
    processing_log = []
    
    # Step 1: Default preprocessing (header validation, null representation cleanup)
    df = preprocess_default(df)
    processing_log.append("✅ Default preprocessing: Header validation and null representation cleanup")
    
    # Step 2: Type conversions
    if type_conversions:
        df, conversion_results = convert_column_types(df, type_conversions)
        successful_conversions = len(conversion_results.get('successful', []))
        processing_log.append(f"✅ Type conversions: {successful_conversions} columns converted")
    
    # Step 3: Per-column null handling
    if null_handling_strategies:
        for column, strategy_info in null_handling_strategies.items():
            if column in df.columns:
                strategy = strategy_info.get('strategy', 'Skip / Leave as is')
                custom_value = strategy_info.get('custom_value')
                df, message = handle_null_values_per_column(df, column, strategy, custom_value)
                processing_log.append(f"✅ Null handling for '{column}': {message}")
    
    # Step 4: Duplicate handling
    if handle_duplicates_flag:
        df, dup_message = handle_duplicates(df, "remove")
        processing_log.append(f"✅ Duplicate handling: {dup_message}")
    
    # Step 5: Stopwords removal
    if remove_stopwords_flag:
        df, stopwords_message = remove_stopwords_from_text_columns(df, True)
        processing_log.append(f"✅ Stopwords removal: {stopwords_message}")
    
    # Step 6: Text normalization
    if normalization_method in ["lemmatize", "stem"]:
        df, norm_message = apply_text_normalization(df, normalization_method)
        processing_log.append(f"✅ Text normalization: {norm_message}")
    
    total_time = time.time() - start_time
    processing_log.append(f"🎯 Total preprocessing time: {total_time:.2f}s")
    
    logger.info(f"Enhanced preprocessing pipeline completed in {total_time:.2f}s")
    return df, processing_log

# Legacy functions for backward compatibility
def preprocess_basic(df: pd.DataFrame, null_handling="keep", fill_value=None):
    """Legacy basic preprocessing - now uses enhanced pipeline"""
    return preprocess_enhanced_pipeline(df)[0]

def preprocess_auto_fast(df: pd.DataFrame):
    """Legacy auto fast preprocessing - now uses enhanced pipeline"""
    return preprocess_enhanced_pipeline(df)[0]

def preprocess_auto_config1(df: pd.DataFrame, null_handling="keep", fill_value=None):
    """Legacy auto config1 preprocessing - now uses enhanced pipeline"""
    return preprocess_enhanced_pipeline(df)[0]

def preprocess_advanced(df: pd.DataFrame,
                        null_handling="keep", fill_value=None,
                        remove_stopwords=False, lowercase=False,
                        text_processing_option="none",  # "none", "stemming", "lemmatization"
                        column_types=None,
                        null_handling_strategies=None,
                        handle_duplicates_flag=True):
    """
    Enhanced advanced preprocessing using the new pipeline
    """
    start_time = time.time()
    
    # Map old parameters to new pipeline parameters
    ...
    df, processing_log = preprocess_enhanced_pipeline(
        df=df,
        type_conversions=column_types,
        null_handling_strategies=null_handling_strategies,
        handle_duplicates_flag=handle_duplicates_flag,
        remove_stopwords_flag=remove_stopwords,
        normalization_method=normalization_method
    )
    
    # Apply additional text cleaning if lowercase is requested
    if lowercase:
        for col in df.select_dtypes(include=["object"]).columns:
            df[col] = clean_text_advanced(df[col], lowercase=True, remove_delimiters=True, remove_whitespace=True)
        processing_log.append("✅ Additional text cleaning: Lowercase and delimiter removal")
    
    # Get conversion results for backward compatibility
    conversion_results = None
    if column_types:
        _, conversion_results = convert_column_types(df, column_types)
    
    total_time = time.time() - start_time
    processing_log.append(f"🎯 Total advanced preprocessing time: {total_time:.2f}s")
    
    logger.info(f"Enhanced advanced preprocessing completed in {total_time:.2f}s")
    logger.info(f"Processing steps: {'; '.join(processing_log)}")
...
# -----------------------------
# 🔹 Embedding + Storage with OpenAI Support
# -----------------------------
def embed_texts(chunks, model_name="all-MiniLM-L6-v2", openai_api_key=None, openai_base_url=None):
    start_time = time.time()
    ...
    if openai_api_key or "text-embedding" in model_name.lower():
        ...
        embeddings = openai_model.encode(chunks)
        model = openai_model
...
def get_system_info():
    ...
    return {
        ...
        "max_recommended_file_size": f"{get_available_memory() * MAX_MEMORY_USAGE / (1024**3):.2f} GB"
    }
...
def run_fast_pipeline(df, db_type="sqlite", db_config=None, file_info=None, 
    ...
    # Auto preprocess with enhanced text cleaning
    df1 = preprocess_auto_fast(df)
    # Default: semantic clustering
    chunks = chunk_semantic_cluster(df1)
    
    # Choose embedding method
    model_choice = "text-embedding-ada-002" if use_openai else "all-MiniLM-L6-v2"
    model, embs = embed_texts(chunks, model_choice, openai_api_key, openai_base_url)
...
def run_config1_pipeline(df, null_handling, fill_value, chunk_method,
    ...
    model, embs = embed_texts(chunks, model_choice, openai_api_key if actual_openai else None, openai_base_url)
...
def process_large_file_fast(file_path: str, **kwargs):
    ...
    def process_batch(batch_df):
        processed_df = preprocess_auto_fast(batch_df)
        chunks = chunk_semantic_cluster(processed_df)
        return chunks
    
    all_chunks = process_large_file_in_batches(file_path, process_batch)
    
    # Embed and store all chunks
    model_choice = kwargs.get('model_choice', 'all-MiniLM-L6-v2')
    use_openai = kwargs.get('use_openai', False)
    openai_api_key = kwargs.get('openai_api_key')
    openai_base_url = kwargs.get('openai_base_url')
    
    model, embs = embed_texts(all_chunks, model_choice, openai_api_key, openai_base_url)
    store = store_faiss(embs)
