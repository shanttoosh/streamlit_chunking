...
if st.session_state.current_mode:
    if st.session_state.current_mode == "fast":
        ...
        apply_default_preprocessing = st.session_state.get("config1_preprocessing", True)

        with tab_preprocess:
            st.markdown("#### 🧹 Default Preprocessing")
            apply_default_preprocessing = st.checkbox(
                "Apply Default Preprocessing",
                value=apply_default_preprocessing,
                help="Automatically clean and normalize data (HTML removal, lowercase, strip whitespace, header normalization)",
                key="config1_preprocessing"
            )
            ...
            if apply_default_preprocessing:
                ...
                **No preprocessing will be applied.**
                Data will be processed as-is with minimal cleaning.
                """)
